...
static void mushspace_min_max_size(
	mush_bounds*, mushspace_consumee*, size_t*, size_t, const mush_aabb*);
	...
	mushspace_consumee* consumee, size_t* used_cells)
{
	for (size_t i = 0; i < space->box_count; ++i) {
		...
		if (!mush_bounds_contains_bounds(consumer, &space->boxen[c].bounds))
			continue;

		subsumees[*slen++] = c;
		mushspace_min_max_size(NULL, consumee, used_cells, c, &space->boxen[c]);
	...
	mushspace_consumee* consumee, size_t* used_cells)
{
	...
	if (*slen - s0 > 1) {
		...
		if (j == s0) {
			...
			const mush_bounds *bounds =
				&space->boxen[candidates[subsumees[s0]]].bounds;
	...
	for (size_t i = s0; i < *slen; ++i) {
		...
		mushspace_min_max_size(consumer, consumee, used_cells,
	...
	mushspace_consumee* consumee, size_t* used_cells)
{
	...
	for (size_t i = 0; i < space->box_count; ++i) {
		...
		if (mush_bounds_overlaps(consumer, &space->boxen[c].bounds))
			continue;
	...
	size_t box_idx, const mush_aabb* box)
{
	...
	mushspace_min_max_size(
		&try_bounds, &try_max, &try_total_size, box_idx, box);
	...
	void(*g)(size_t, void*))
{
	...
	for (;;) next_pos: {
		if (mush_staticaabb_contains(pos)) {
			if (mushspace_map_in_static(space, &aabb->bounds, &pos, fg, f))
				return;
			else
				goto next_pos;
		}
		...
		for (size_t b = 0; b < space->box_count; ++b) {
			...
			if (!mush_bounds_contains(&box->bounds, pos))
				continue;

			if (mushspace_map_in_box(space, &aabb->bounds, &pos, box, b, fg, f))
				return;
			else
				goto next_pos;
		}
	...
	void(*g)(size_t, void*))
{
	...
	for (;;) next_pos: {
		if (mush_staticaabb_contains(pos)) {
			if (mushspace_mapex_in_static(space, &aabb->bounds, &pos, fg, f))
				return;
			else
				goto next_pos;
		}
		...
		for (size_t b = 0; b < space->box_count; ++b) {
			...
			if (mushspace_mapex_in_box(space, &aabb->bounds, &pos, box, b, fg, f))
				return;
			else
				goto next_pos;
		}
